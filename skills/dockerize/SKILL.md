...
- **Compose wrappers**: create `compose-up.sh`, `compose-down.sh`, and `compose-logs.sh` only when there is a long-lived service or daemon. Always create `compose-test.sh`, but make it report that no tests were found and exit successfully when there is no runnable test service. Create `compose-export.sh` when a named volume contains outputs that users need to export to the host.

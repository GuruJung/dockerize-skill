...
1. Before writing Docker files, inspect the project's existing `compose.yaml`, `docker-compose.yaml`, wrapper scripts, `.env.example`, dependency manifests, entrypoints, default commands, scripts, expected outputs, large data/model folders, side services, ports, GPU needs, and Hugging Face usage. Determine whether each process keeps running while waiting for requests or work, or exits after completing its work.
...
- **Project and volume names**: do not add `COMPOSE_PROJECT_NAME` to `.env.example` or force a project name with top-level Compose `name:`. Use the `COMPOSE_PROJECT_NAME` that Compose determines automatically, and make shareable volume names overrideable per volume, such as `${RESULTS_VOLUME_NAME:-${COMPOSE_PROJECT_NAME}_results}`. Treat Hugging Face caches as an exception under the dedicated policy that follows.
- **Hugging Face cache and authentication**: when a project uses Hugging Face, store the full `HF_HOME` in a shared external named volume whose default is `${HF_CACHE_VOLUME_NAME:-<main-project-name>_hf-cache}`, calculated from the main worktree's default Compose project name. Do not derive this default from the current worktree's `COMPOSE_PROJECT_NAME`, a transient `-p`, or the shell's `COMPOSE_PROJECT_NAME`. Ask the user for a stable name if the `refs/heads/main` worktree cannot be found or its root basename violates Compose project-name rules. Inline volume inspection and creation in every wrapper that creates containers; do not generate a separate helper. Use a `${HF_CACHE_DIR:-${HOME}/.cache/huggingface}` read-write bind only when the user needs to reuse an existing host cache, share with host tools, manage files directly, or preserve host-account ownership; on native Linux run it with `HOST_UID`/`HOST_GID`. Pass `HF_TOKEN` only as an optional environment or `.env` value, and never record it in an image, repository, or validation log. Warn that sharing the full `HF_HOME` can persist or expose a stored token and that a set `HF_TOKEN` overrides the stored token. Follow `references/ml.md` for the detailed patterns.
